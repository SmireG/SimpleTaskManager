...
int main()
{
	//cout << sizeof(Process)<<" "<<sizeof(WCHAR)<<" "<<sizeof(float)<<" "<<sizeof(int64_t)<<" "<<sizeof(SIZE_T)<<" "<<sizeof(ULONGLONG)<<" "<<sizeof(DWORD)<<"\n";
...
void CALLBACK GetInfo(HWND hwnd, UINT message, UINT idTimer, DWORD dwTime)
{
	...
	duration<double, std::milli> time_span = t1 - last;
